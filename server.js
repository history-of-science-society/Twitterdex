...
async function start() {
  try {
    ...
    let pageNos = await fetch(
      `https://www.formstack.com/api/v2/form/${formId}/submission.json?oauth_token=${oauth_token}`
    )
    ...
    for (let i = 1; i <= pageNos; i++) {
      let raw = await fetch(
        `https://www.formstack.com/api/v2/form/${formId}/submission.json?page=${i}&data=1&oauth_token=${oauth_token}`
      );
      let res = await raw.json();
      console.log(
        "\x1b[32m",
        `Fetching form responses from the database (${i} of ${pageNos})...`
      );
    ...
    for (set in data) {
      ...
      for (el in data[set]) {
        ...
        let obj = new Twitterstorian(
          data[set][el].data[73000979].value,
          data[set][el].data[73000996].value,
          data[set][el].data[73001016].value,
          data[set][el].data[73001024].value
        );

        console.log(
          "\x1b[32m",
          `Constructing object and fetching data for ${obj.name}`
        );
        ...
        let encodedUrl = `https://publish.twitter.com/oembed?url=https%3A%2F%2Ftwitter.com%2F${obj.handleRaw}%2Fstatus%2F${latestTweetId}&omit_script=1`;
    ...
    fs.readFile("./src/hbs/index.hbs", function (err, data) {
      if (!err) {
        console.log(
          "\x1b[32m",
          `Creating new HSS Twitterdex. A Crowd-Sourced Directory of History of Science Twitterstorians. Now with ${twitterstorians.length} Twitterstorians. Have a great day! 😄`
        );
        const src = data.toString();
        const template = Handlebars.compile(src);
        const html = template(newObj);
        fs.writeFile("./twitterdex/index.html", html, (err) =>
          console.log(err)
        );
      } else {
...
function nameParse(input) {
  ...
  const name =
    firstName[1]
      .trim()
      .toLowerCase()
      .replace(/(^\w)|\s(\w)|-(\w)/g, (x) => x.toUpperCase()) +
    " " +
    lastName[1]
      .trim()
      .toLowerCase()
